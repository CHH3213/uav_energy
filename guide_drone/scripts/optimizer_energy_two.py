...
def op_fun_max(args):
    k_t = args
    fun = lambda x: k_t*np.max([np.linalg.norm(np.array([x[0],x[1],x[2]]))**1.5, np.linalg.norm(np.array([x[3],x[4],x[5]]))**1.5])
    return fun
def op_fun_sum(args):
    k_t = args
    fun = lambda x: k_t*np.sum([np.linalg.norm(np.array([x[0],x[1],x[2]]))**1.5, np.linalg.norm(np.array([x[3],x[4],x[5]]))**1.5])
    return fun
def con(args):
    #作用点：顶点，质心点,重力
    triangle_angle_1, triangle_angle_2,payload_center,payload_gravity= args
    # 公式12
    constrants = (
        {'type':'eq','fun':lambda x: (np.cross(triangle_angle_1,np.array([x[0],x[1],x[2]]))+np.cross(triangle_angle_2,np.array([x[3],x[4],x[5]]))+np.cross(payload_center,payload_gravity))[0]},
        {'type':'eq','fun':lambda x: (np.cross(triangle_angle_1,np.array([x[0],x[1],x[2]]))+np.cross(triangle_angle_2,np.array([x[3],x[4],x[5]]))+np.cross(payload_center,payload_gravity))[1]},
        {'type':'eq','fun':lambda x: (np.cross(triangle_angle_1,np.array([x[0],x[1],x[2]]))+np.cross(triangle_angle_2,np.array([x[3],x[4],x[5]]))+np.cross(payload_center,payload_gravity))[2]},
        ...
        # {'type':'eq','fun':lambda x: triangle_angle_1[0]*x[1]-x[0]*triangle_angle_1[1]+triangle_angle_2[0]*x[4]-x[3]*triangle_angle_2[1]+triangle_angle_3[0]*x[7]-x[6]*triangle_angle_3[1]+payload_center[0]*payload_gravity[1]-payload_center[1]*payload_gravity[0]},                
                    {'type':'eq','fun':lambda x: x[0]+x[3]+payload_gravity[0]},
                    {'type':'eq','fun':lambda x: x[1]+x[4]+payload_gravity[1]},
                    {'type':'eq','fun':lambda x: x[2]+x[5]+payload_gravity[2]}
                    )
    return constrants
...
def minimizeForce(allArgs):
    triangle_angle_1, triangle_angle_2,payload_center,payload_gravity,k_t= allArgs
    args = [triangle_angle_1, triangle_angle_2,payload_center,payload_gravity]
    cons = con(args)

    x0 = np.asarray([0.0,0.0,1/2, 0.,0.,1/2])
    # print(x0[0][0])
    ...
    # print(res_max.x)
    # print(res_sum.x)
    print('optimizeresult', res_max.success)
    print('optimizeresult',res_sum.success)
    return v_max,v_sum,result_max,result_sum,res_max.success,res_sum.success
...
if __name__ == "__main__":
    ...
    payload_pos = np.array([-0.25,0,1])
    payload_gravity = np.array([0.5,0.,-1])
    # payload_gravity = np.array([0.,0.,-1])
    ''' 
    ...
    payload_gravity = np.array([0,0,-0.3*9.8])
    payload_pos = np.array([0.25,0,0.6])
    fixed1 = np.array([0.5,0,0.6])
    fixed2 = np.array([-0.5,0,0.6])
    args = (fixed1, fixed2, payload_pos,payload_gravity ,4.328)
    # print(np.array([0,0,-1])[2])
    v_max,v_sum,result_max,result_sum  ,_,_= minimizeForce(args)
    # print(v_max)
    sum_norm_1,sum_norm_2 =np.linalg.norm(v_sum[0:3]),np.linalg.norm(v_sum[3:6])
    max_norm_1,max_norm_2 =np.linalg.norm(v_max[0:3]),np.linalg.norm(v_max[3:6])
