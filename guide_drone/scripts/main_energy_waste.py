...
import os
import time
from optimizer_energy import minimizeForce
...
class energy_waste:
    def __init__(self):
        ...
        self.payload_pos = np.array([0,3/4*np.sqrt(3),1])
        self.payload_vector = np.array([1,0,-1])
        # 绳子方向,指向等边三角形顶点：（1，0，1），（-1，0，1），（0，np.sqrt(3),1)
        self.triangle_angle_1 = np.array([1,0,1])
        self.triangle_angle_2 = np.array([-1,0,1])
        self.triangle_angle_3 = np.array([0,np.sqrt(3),1])
        ...
        #无人机相对世界坐标系的初始点
        self.world_drone1 = np.array([0.3*np.sqrt(3),0.3,0])
        self.world_drone2 = np.array([-0.3*np.sqrt(3),-0.3,0])
        self.world_drone3 = np.array([0,0,0.6])
        #无人机重力矢量
        self.gravity_drone1 = np.array([0,0,-9.8*1.4])
        self.gravity_drone2 = np.array([0,0,-9.8*1.4])
        self.gravity_drone3 = np.array([0,0,-9.8*1.4])
        # 初始推力矢量--竖直向上 用四元数，后面补了0
        self.init_drone_thrust = np.array([0,0,1,0])  

        self.saveData_dir = "../log"
        ...
        self.state_sub3 = rospy.Subscriber('/drone3/mavros/state', State, self.state_callback3)
        ...
        self.setpoint_raw_pub3 = rospy.Publisher("/drone3/mavros/setpoint_raw/attitude",AttitudeTarget,queue_size=1)
    ...
    def set_arm(self, arm):
        ...
        counter = 0
        while self.mavros_state1.armed != arm or self.mavros_state2.armed != arm or self.mavros_state3.armed != arm:
            rospy.logerr("failed to send arm command")
            self.arming_client1(arm)
            self.arming_client2(arm)
            self.arming_client3(arm)
            counter = counter + 1
            rate.sleep()
    ...
    def set_mode(self, mode):
        ...
        rate = rospy.Rate(loop_freq)
        counter = 0
        while self.mavros_state1.mode != mode or self.mavros_state2.mode != mode or self.mavros_state3.mode != mode:
            print('current mode', self.mavros_state1.mode, self.mavros_state2.mode,self.mavros_state3.mode)
            rospy.logerr("failed to send set mode command")
            self.set_mode_client1(0, mode)
            self.set_mode_client2(0, mode)
            self.set_mode_client3(0, mode)
            counter = counter + 1
            rate.sleep()
    ...
    def get_posByMavros(self):
        ...
        pos_drone3 = np.array([self.local_position3.pose.position.x,self.local_position3.pose.position.y,self.local_position3.pose.position.z])
        ...
        ori_drone3 = np.array([self.local_position3.pose.orientation.x,self.local_position3.pose.orientation.y,self.local_position3.pose.orientation.z,self.local_position3.pose.orientation.w])

        return [pos_drone1,pos_drone2,pos_drone3,ori_drone1,ori_drone2,ori_drone3]

    

    def estimate_fixed_point(self,thrust1_mod,thrust2_mod,thrust3_mod,bias_pos_drone1,bias_pos_drone2,bias_pos_drone3):
        '''
        由无人机位置估算出固定点的位置
        '''
        # 无人机位置
        list_pos_drone1,list_pos_drone2,list_pos_drone3,list_ori_drone1,list_ori_drone2,list_ori_drone3 = 0,0,0,0,0,0
        count = 0
        start = time.time()
        while time.time()-start<3:
            [pos_drone1,pos_drone2,pos_drone3,ori_drone1,ori_drone2,ori_drone3] = self.get_posByMavros()  # 相对自身原点的位置
            count +=1
            list_ori_drone1 += ori_drone1
            list_ori_drone2 += ori_drone2
            list_ori_drone3 += ori_drone3
            list_pos_drone1 += pos_drone1
            list_pos_drone2 += pos_drone2
            list_pos_drone3 += pos_drone3

        ori_drone1,ori_drone2, ori_drone3,pos_drone1,pos_drone2,pos_drone3= list_ori_drone1/count,list_ori_drone2/count,list_ori_drone3/count,list_pos_drone1/count,list_pos_drone2/count,list_pos_drone3/count
        ...
        q3 = rotated_vector(ori_drone3,self.init_drone_thrust)

        thrust1_unit = q1[0:3]/np.linalg.norm(q1[0:3])
        thrust2_unit = q2[0:3]/np.linalg.norm(q2[0:3])
        thrust3_unit = q3[0:3]/np.linalg.norm(q3[0:3])
        thrust1_vector = np.array(thrust1_unit)*thrust1_mod
        thrust2_vector = np.array(thrust2_unit)*thrust2_mod
        thrust3_vector = np.array(thrust3_unit)*thrust3_mod
        print('**************thrust_vec**************')
        print('thrust1_vector',thrust1_vector)
        print('thrust2_vector',thrust2_vector)
        print('thrust3_vector',thrust3_vector)
        print('$$$$$$$$$$$$$$$$$$$$$$$$$$')

        # 绳子拉力向量
        self.cable1_vector = thrust1_vector -self.gravity_drone1
        self.cable2_vector = thrust2_vector -self.gravity_drone2
        self.cable3_vector = thrust3_vector -self.gravity_drone3
        #单位向量
        ...
        cable3_u = self.cable3_vector/np.linalg.norm(self.cable3_vector)
        ...
        fixed_point3  = np.array(pos_drone3)+np.array(bias_pos_drone3) +self.world_drone3-  cable3_u * self.cable_length
        print('++++++++++++++fixed point++++++++++++++')
        print('fixed_point1',fixed_point1)
        print('fixed_point2',fixed_point2)
        print('fixed_point3',fixed_point3)
        print('++++++++++++++++++++++++++++')

        # 返回世界坐标系下的坐标
        return fixed_point1,fixed_point2,fixed_point3

    def estimate_payload_vector(self,cable1,cable2,cable3,fixed_point1,fix_point2,fix_point3,payload_point):
        '''
        已知绳子拉力向量和作用点，估计负载向量
        '''
        ...
        payload_vector = -(cable1 + cable2 + cable3)
    ...
    def compute_optimize(self,args):
        ...
        u3_max = np.array(v_max[6:9])/np.linalg.norm(v_max[6:9])
        u1_sum = np.array(v_sum[0:3])/np.linalg.norm(v_sum[0:3])
        u2_sum = np.array(v_sum[3:6])/np.linalg.norm(v_sum[3:6])
        u3_sum = np.array(v_sum[6:9])/np.linalg.norm(v_sum[6:9])
        # 最大化最小化后的位置  公式15
        ...
        m_position_drone3 = u3_max * self.cable_length+self.triangle_angle_3
        # 求和最小化后的位置
        ...
        s_position_drone3 = u3_sum * self.cable_length+self.triangle_angle_3
        return m_position_drone1, m_position_drone2, m_position_drone3,s_position_drone1,s_position_drone2,s_position_drone3
    ...
    def run(self):
        ...
        [self.init_drone1,self.init_drone2,self.init_drone3,_,_,_] = self.get_posByMavros()# 初始坐标
        print('^^^^^^^^^initial^^^^^^^^^^')
        print('self.init_drone1',self.init_drone1)
        print('self.init_drone2',self.init_drone2)
        print('self.init_drone3',self.init_drone3)
        ...
        if flag ==2:
            ...
            while True:
                ...
                self.takeoff_service3(altitude=0.5+self.init_drone3[2])
                [drone1,drone2,drone3,_,_,_] = self.get_posByMavros()
                print('self.init_drone1',drone1)
                print('self.init_drone2',drone2)
                print('self.init_drone3',drone3)
                print('==============================')
                if np.abs(drone1[2]-0.5-self.init_drone1[2]) <0.2 and np.abs(drone2[2]-0.5-self.init_drone2[2]) <0.2 and np.abs(drone3[2]-0.5-self.init_drone3[2]) <0.2:
                    break
        ...
        if flag ==2:
            ...
            while True:
                ...
                self.takeoff_service3(altitude=1+self.init_drone3[2])
                [drone1,drone2,drone3,_,_,_] = self.get_posByMavros()
                print('self.init_drone1',drone1)
                print('self.init_drone2',drone2)
                print('self.init_drone3',drone3)
                print('=========================')
                if np.abs(drone1[2]-1-self.init_drone1[2]) <0.2 and np.abs(drone2[2]-1-self.init_drone2[2]) <0.2 and np.abs(drone3[2]-1-self.init_drone3[2]) <0.2:
                    break
        ...
        if flag ==2:
            ...
            while True:
                ...
                self.takeoff_service3(altitude=1.5+self.init_drone3[2])
                [drone1,drone2,drone3,_,_,_] = self.get_posByMavros()
                print('self.init_drone1',drone1)
                print('self.init_drone2',drone2)
                print('self.init_drone3',drone3)
                print('=======================')
                if np.abs(drone1[2]-1.5-self.init_drone1[2]) <0.2 and np.abs(drone2[2]-1.5-self.init_drone2[2]) <0.2 and np.abs(drone3[2]-1.5-self.init_drone3[2]) <0.2:
                    break
        while True:
            ...
            if flag == 2:
                ...
                mod_thrust3 = 0.008138 * np.sum(self.rc_states3.channels) -4 * 9.211

                # 返回平台固定点位置--实际位置
                self.triangle_angle_1, self.triangle_angle_2, self.triangle_angle_3 = self.estimate_fixed_point( thrust1_mod=mod_thrust1,thrust2_mod=mod_thrust2,thrust3_mod=mod_thrust3,bias_pos_drone1= self.init_drone1,bias_pos_drone2= self.init_drone2,bias_pos_drone3= self.init_drone3)
                # 一开始需要估计负载向量
                payload_vector = self.estimate_payload_vector(self.cable1_vector,self.cable2_vector,self.cable3_vector,self.triangle_angle_1,self.triangle_angle_2,self.triangle_angle_3,self.payload_pos)
            ...
            if flag == 2:
                ...
                # 优化
                args = (self.triangle_angle_1,self.triangle_angle_2,self.triangle_angle_3, self.payload_pos, payload_vector ,self.k_t)
                m_position_drone1, m_position_drone2, m_position_drone3,s_position_drone1,s_position_drone2,s_position_drone3 = self.compute_optimize(args)  #实际位置-世界坐标系
                print('************world frame*************')
                print('m_position_drone1',m_position_drone1)
                print('m_position_drone2',m_position_drone2)
                print('m_position_drone3',m_position_drone3)
                print('**********************')
                #这次需要飞的位置---最小化最大
                temp_drone1 = m_position_drone1 - self.init_drone1-self.world_drone1
                temp_drone2 = m_position_drone2 - self.init_drone2-self.world_drone2
                temp_drone3 = m_position_drone3 - self.init_drone3-self.world_drone3
                print('===========first==========')
                print('first_temp_drone1',temp_drone1)
                print('first_temp_drone2',temp_drone2)
                print('first_temp_drone3',temp_drone3)
            flag = input('first optimized takeoff==>continue:1, over:2')
            if flag == 2:
                break
            else:
                ...
                pose_stamped_3 = self.goto_xyz_rpy(temp_drone3[0], temp_drone3[1], temp_drone3[2], 0, 0, 0)

                self.local_pos_pub1.publish(pose_stamped_1)
                # rospy.sleep(0.0005)
                self.local_pos_pub2.publish(pose_stamped_2)
                self.local_pos_pub3.publish(pose_stamped_3)
            ...
            temp_drone3 = s_position_drone3 - self.init_drone3-self.world_drone3
            print('===========second==========')
            print('second_temp_drone1',temp_drone1)
            print('second_temp_drone2',temp_drone2)
            print('second_temp_drone3',temp_drone3)
