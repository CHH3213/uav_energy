...
class energy_waste:
    def __init__(self):
        ...
        self.payload_pos = np.array([0.0,0,1])
        ...
        #无人机相对世界坐标系的初始点
        self.world_drone1 = np.array([0.8,0.,0])
        self.world_drone2 = np.array([-0.8,0,0])
        #无人机重力矢量
    ...
    def goto_xyz_rpy(self, x, y, z, ro, pi, ya):
        ...
        pose.position.y = y
        pose.position.z = z
        # quat = euler_to_quaternion(ro,pi,ya+np.pi/2)
        quat = euler_to_quaternion(ro,pi,ya)
    ...
    def estimate_fixed_point(self,thrust1_mod,thrust2_mod,bias_pos_drone1,bias_pos_drone2):
        ...
        count = 0 
        start = time.time()
        list_pos_drone1,list_pos_drone2,list_ori_drone1,list_ori_drone2 = 0,0,0,0
        ...
        # ori_drone1,ori_drone2,pos_drone1,pos_drone2= list_ori_drone1/count,list_ori_drone2/count,list_pos_drone1/count,list_pos_drone2/count
        [pos_drone1,pos_drone2,ori_drone1,ori_drone2] = self.get_posByMavros()  # 相对自身原点的位置
        ...
        print('=============')
        print('ori_drone1',ori_drone1)
        print('ori_drone2',ori_drone2)
        print('===============')
        thrust1_unit = q1[0:3]/np.linalg.norm(q1[0:3])
        thrust2_unit = q2[0:3]/np.linalg.norm(q2[0:3])
        thrust1_unit[1] = 0
        thrust2_unit[1] = 0
        thrust1_unit = thrust1_unit/np.linalg.norm(thrust1_unit)
        thrust2_unit = thrust2_unit/np.linalg.norm(thrust2_unit)
        self.thrust1_vector = np.array(thrust1_unit)*thrust1_mod
        self.thrust2_vector = np.array(thrust2_unit)*thrust2_mod
        print('thruat_vector',self.thrust1_vector)
        print('thruat_vector',self.thrust2_vector)
        # 绳子拉力向量
        self.cable1_vector =self. thrust1_vector +self.gravity_drone1
        self.cable2_vector = self.thrust2_vector +self.gravity_drone2
        #单位向量
        ...
        print('****************')
        print('cable1_vector', self.cable1_vector)
        print('cable2_vector', self.cable2_vector)
        # 固定点计算  # 公式10
    ...
    def compute_optimize(self,args):
        ...
        # 返回优化后的绳子拉力向量
        v_max,v_sum,result_max,result_sum, r_max,r_sum  = minimizeForce(args)
        # 单位化
        ...
        return m_position_drone1, m_position_drone2,s_position_drone1,s_position_drone2,r_max,r_sum
    ...
    def run(self):
        ...
        if flag ==2:
            ...
            while True:
                ...
                # if np.abs(drone1[2]-1-self.init_drone1[2]) <0.2  :
                    break
            start = time.time()
            while time.time()-start<3:
                temp_drone1 = np.array([-0.0,0,1.0])+np.array(self.init_drone1)
                temp_drone2 = np.array([0.1,0,1.0])+np.array(self.init_drone2)
                print('===========modify takeoff==========')
                print('temp_drone1',temp_drone1)
                print('temp_drone2',temp_drone2)
                
                pose_stamped_1 = self.goto_xyz_rpy(temp_drone1[1], temp_drone1[0], temp_drone1[2], 0, 0, 0)
                pose_stamped_2 = self.goto_xyz_rpy(temp_drone2[1], temp_drone2[0], temp_drone2[2], 0, 0, 0)
                ...
                [drone1,drone2,_,_] = self.get_posByMavros()
                print('---------------------')
                print('drone1:takeoff1',drone1)
                print('drone2:takeoff1',drone2)
                print('=========================')
        ...
        #         print('mode_thrust1',mod_thrust1 )
        #         print('mode_thrust1',mod_thrust2 )
        #         print('============================')
        #         # 返回平台固定点位置--实际位置
        ...
        #         print('fixedpoint1',self.triangle_angle_1)
        #         print('fixedpoint2',self.triangle_angle_2)
        #         print('------------------------------------')
        #         # 一开始需要估计负载向量
        #     )
        #         self.payload_pos [2] = (self.triangle_angle_1[2]+self.triangle_angle_2[2])/2
        ...
        #     thrust_before = pow(np.linalg.norm(self.thrust1_vector),1.5)+pow(np.linalg.norm(self.thrust2_vector),1.5)
        #     print('power_before',thrust_before )
        ...
        #         # 优化
        #         while True:
        #             # self.triangle_angle_1 = np.array(0.5,0,0.6)
        #             # self.triangle_angle_2 = np.array(-0.5,0,0.6)
        #             # self.payload_pos = np.array([0.25,0,0.6])
        #             # payload_vector = np.array([0,0,-0.3*9.8])
        #             args = (self.triangle_angle_1,self.triangle_angle_2, self.payload_pos, payload_vector ,self.k_t)

        #             m_position_drone1, m_position_drone2,s_position_drone1,s_position_drone2,result_sum,result_max = self.compute_optimize(args)  #实际位置-世界坐标系
        #             if result_sum==True or result_sum==False :
        #                 break
            #下次需要飞的位置---最小化求和
        ...
        thrust1_unit = thrust1_unit/np.linalg.norm(thrust1_unit)
        thrust2_unit = thrust2_unit/np.linalg.norm(thrust2_unit)
        self.thrust1_vector = np.array(thrust1_unit)*mod_thrust1
        self.thrust2_vector = np.array(thrust2_unit)*mod_thrust2
        print('====优化前========')
        print('thrust1_vector',self.thrust1_vector)
        print('thrust2_vector',self.thrust2_vector)
        thrust_before = pow(np.linalg.norm(self.thrust1_vector),1.5)+pow(np.linalg.norm(self.thrust2_vector),1.5)
        print('power_before',thrust_before )
        temp_drone1 = np.array([0.4,0,1.2])+np.array(self.init_drone1)
        temp_drone2 = np.array([-0.4,0,1.2])+np.array(self.init_drone2)
        print('===========optimize takeoff==========')
        ...
        if flag == 2:
            ...
            while True:
                ...
                # rospy.sleep(0.0005)
                self.local_pos_pub2.publish(pose_stamped_2)
                flag = input('继续？==>continue:1, over:2')
                if flag == 2:
                    break
                elif flag ==1:
                    continue
            ...
            print('thrust1_vector',self.thrust1_vector)
            print('thrust2_vector',self.thrust2_vector)
            thrust_after = pow(np.linalg.norm(self.thrust1_vector),1.5)+pow(np.linalg.norm(self.thrust2_vector),1.5)
            print('power_after',thrust_after )
