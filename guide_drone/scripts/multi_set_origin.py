...
if __name__=="__main__":
    try:
        ...
        mavlink_pub1 = rospy.Publisher("/drone1/mavlink/to", Mavlink, queue_size=20)
        mavlink_pub2 = rospy.Publisher("/drone2/mavlink/to", Mavlink, queue_size=20)
        # mavlink_pub3 = rospy.Publisher("/drone3/mavlink/to", Mavlink, queue_size=20)

        # # Set up mavlink instance
        # f = fifo()
        #
        # mav1 = MAV_APM.MAVLink(f, srcSystem=1, srcComponent=1)
        # mav2 = MAV_APM.MAVLink(f, srcSystem=2, srcComponent=1)

        # Set up mavlink instance
        f1 = fifo()
        f2 = fifo()

        mav1 = MAV_APM.MAVLink(f1, srcSystem=1, srcComponent=1) # this parammeter ?
        mav2 = MAV_APM.MAVLink(f2, srcSystem=2, srcComponent=1)

        wait to initialize
        while mavlink_pub1.get_num_connections() <= 0:
            pass

        # wait to initialize
        while mavlink_pub2.get_num_connections() <= 0:
            pass
   
        for _ in range(2):
            rospy.sleep(1)
            set_global_origin(mav1, mavlink_pub1)
            set_home_position(mav1, mavlink_pub1)
            set_global_origin(mav2, mavlink_pub2)
            set_home_position(mav2, mavlink_pub2)
