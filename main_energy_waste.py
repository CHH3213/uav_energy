...
class energy_waste:
    def __init__(self):
        ...
        self.gravity_drone3 = np.array([0,0,-9.8*1.888])
        # 初始推力矢量--竖直向上 用四元数，后面补了0
        self.init_drone_thrust = np.array([0,0,1,0])  

        self.saveData_dir = "/home/firefly/chh_ws/src/ugv_uav/scripts/data"
    ...
    def run(self):
        ...
        self.takeoff_service2(altitude=1.5+self.init_drone1[2])
        self.takeoff_service3(altitude=1.5+self.init_drone1[2])

        while True:
            flag = eval(input('estimate_fixed_point==>continue:1, over:2'))
            if flag == 2:
                break
            else:
            ...
            flag = eval(input('optimize==>continue:1, over:2'))
            if flag == 2:
                break
            else:
                # 优化
            ...
            flag = eval(input('first optimized takeoff==>continue:1, over:2'))
            if flag == 2:
                break
            else:
            ...
            flag = eval(input('second optimized takeoff==>continue:1, over:2'))
            if flag == 2:
                break
            else:
                pose_stamped_1 = self.goto_xyz_rpy(temp_drone1[0], temp_drone1[1], temp_drone1[2], 0, 0, 0)
